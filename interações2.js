...
setinhaquiz.addEventListener('click', function(){
  ...
  if(perguntaatual == 4){
    ...
    if(pont <= 2){
      ...
      ponttxt.innerHTML = pont + "/ 4 <br> Estude mais sobre Pókemons e volte novamente! </br>";
    }
    else if(pont == 3){
      ...
      ponttxt.classList.remove("medio");
      ponttxt.classList.remove("ruim");
      ponttxt.innerHTML = pont + "/ 4 <br> Você é um expert nisso! </br>";
    }
    setinhaquiz.classList.toggle("escondido");
  ...
  if(perguntaatual == 1){
    if("Master Ball" == getRadioValor("p1")){
      pont++;
    }
  }
  else if(perguntaatual == 2){
    if("Pikachu" == getRadioValor("p2")){
      pont++;
    }
  }
  else if(perguntaatual == 3){
    if("Caterpie" == getRadioValor("p3")){
      pont++;
    }
  }
});
